...
def top_left_crop(img, cropsize, default_value):
    ...
    if len(img.shape) == 2:
        container = np.ones((crop_h, crop_w), img.dtype)*default_value
    else:
        container = np.ones((crop_h, crop_w, img.shape[2]), img.dtype)*default_value

    container[:ch, :cw] = img[:ch, :cw]
    return container

def add_padding(x, border = 4):
    # zero padding
    # pad_x = np.zeros((32 + borrder * 2, 32 + border * 2, 3))
